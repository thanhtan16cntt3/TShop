...
@section('title', 'TShop | Post categories')
...
@section('content-main')
    <div class="card shadow mb-4">
        <div class="card-header">
            <h4 class="font-weight-bold text-primary float-left">Post categories</h4>
            <a href="{{ route('post-categories.create') }}" class="btn btn-primary btn-sm float-right" data-toggle="tooltip" data-placement="bottom" title="Add post category"><i class="fas fa-plus"></i> Add Post category</a>
        </div>
        <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($postCategories as $key => $category)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('post-categories.edit', $category->id) }}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                                        <form method="POST" action="{{ route('post-categories.destroy', $category->id) }}">
                                            @csrf
                                            @method('DELETE')
