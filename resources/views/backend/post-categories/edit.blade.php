...
@section('content-main')
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('post-categories.update', $postCategory->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="title">Title <span class="text-danger">*</span></label>
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" placeholder="Enter title" value="{{ old('title', $postCategory->title) }}">
                    @error('title')
